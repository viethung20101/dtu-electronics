  ...
  it('no wire drives a slide-switch terminal from a gate output', () => {
    ...
    for (const ex of digitalExamples) {
      ...
      for (const w of ex.wires) {
        ...
        const endIsGateY = endC?.type.startsWith('velxio-logic-gate-') && w.end.pinName === 'Y';
        ...
        if ((startIsGateY && endIsSwitch) || (endIsGateY && startIsSwitch)) {
          failures.push(`${ex.id}: wire ${w.id} drives a switch terminal from a gate Y output`);
        }
      }
    }
    expect(failures).toEqual([]);
...
function withSwitchValues(ex: ExampleProject, value: 0 | 1): ExampleProject {
  return {
    ...
    components: ex.components.map((c) =>
      c.type === 'wokwi-slide-switch' ? { ...c, properties: { ...c.properties, value } } : c,
    ),
  };
}
...
describe('digitalExamples — live ngspice convergence', () => {
  for (const ex of digitalExamples) {
    it(`${ex.id} solves with switches all-LOW and all-HIGH`, { timeout: 30_000 }, async () => {
      const low = await solveExample(withSwitchValues(ex, 0));
      expect(low.variableNames.length, `${ex.id} (all LOW): no variables returned`).toBeGreaterThan(
        0,
      );
...
describe('digitalExamples — truth table spot checks', () => {
  it('AND gate: only HIGH when both inputs HIGH', { timeout: 30_000 }, async () => {
  ...
  it('XOR gate: HIGH when inputs differ', { timeout: 30_000 }, async () => {
  ...
  it('NAND-built XOR: matches XOR truth table', { timeout: 30_000 }, async () => {
  ...
  it('Half adder: SUM = A XOR B, CARRY = A AND B', { timeout: 30_000 }, async () => {
  ...
  it('Full adder: SUM and Cout match A+B+Cin', { timeout: 30_000 }, async () => {
    ...
    for (let a = 0 as 0 | 1; a <= 1; a = (a + 1) as 0 | 1) {
      for (let b = 0 as 0 | 1; b <= 1; b = (b + 1) as 0 | 1) {
        for (let ci = 0 as 0 | 1; ci <= 1; ci = (ci + 1) as 0 | 1) {
          ...
          if (expSum) expect(isHIGH(vS), `FA(${a},${b},${ci}).SUM → ${vS}V (want HIGH)`).toBe(true);
          else expect(isLOW(vS), `FA(${a},${b},${ci}).SUM → ${vS}V (want LOW)`).toBe(true);
          if (expCo) expect(isHIGH(vC), `FA(${a},${b},${ci}).Cout → ${vC}V (want HIGH)`).toBe(true);
  ...
  it('Majority voter: HIGH iff ≥ 2 of 3 inputs HIGH', { timeout: 30_000 }, async () => {
  ...
  it('2-to-1 MUX: SEL routes D0 or D1 to Y', { timeout: 30_000 }, async () => {
    ...
    for (const [sel, d0, d1, expected] of cases) {
      ...
      if (expected) expect(isHIGH(v), `MUX(sel=${sel},d0=${d0},d1=${d1}) → ${v}V HIGH`).toBe(true);
  ...
  it('Hamming(7,4): p1 = D0 XOR D1 XOR D3 (spot check)', { timeout: 30_000 }, async () => {
    ...
    for (const [d0, d1, d2, d3, expected] of cases) {
      ...
      if (expected) expect(isHIGH(v), `p1(D=${d3}${d2}${d1}${d0}) → ${v}V (want HIGH)`).toBe(true);
...
describe('digitalExamples — advanced truth tables', () => {
  it('4-bit ripple-carry adder: sum bits match A+B+Cin', { timeout: 60_000 }, async () => {
    ...
    for (const tc of cases) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        else expect(isLOW(v), `RCA(${tc.A}+${tc.B}+${tc.Ci}).S${i} → ${v}V (want LOW)`).toBe(true);
      }
      const cout = (total >> 4) & 1;
      const vC = await gateOutputV(ex, 'orC_3', switches);
      if (cout) expect(isHIGH(vC), `RCA(${tc.A}+${tc.B}).Cout → ${vC}V (want HIGH)`).toBe(true);
      else expect(isLOW(vC), `RCA(${tc.A}+${tc.B}).Cout → ${vC}V (want LOW)`).toBe(true);
    }
  });

  it('2-bit × 2-bit multiplier: product bits match A × B', { timeout: 30_000 }, async () => {
    ...
    for (const { A, B } of cases) {
      ...
      for (const [gate, bit] of probes) {
        ...
        if (want) expect(isHIGH(v), `${A}×${B}=${product}, P${bit} → ${v}V (want HIGH)`).toBe(true);
        else expect(isLOW(v), `${A}×${B}=${product}, P${bit} → ${v}V (want LOW)`).toBe(true);
      }
    }
  });

  it('4-bit popcount: bit count of X3..X0 matches output', { timeout: 30_000 }, async () => {
    ...
    for (const { in: inVal, count } of cases) {
      ...
      for (const [gate, bit] of probes) {
        ...
        if (want)
          expect(isHIGH(v), `popcount(${inVal.toString(2)})=${count}, bit${bit} → ${v}V HIGH`).toBe(
            true,
          );
        else
          expect(isLOW(v), `popcount(${inVal.toString(2)})=${count}, bit${bit} → ${v}V LOW`).toBe(
            true,
          );
      }
    }
  ...
  it('3-to-8 decoder: exactly Y_i is HIGH for input i', { timeout: 60_000 }, async () => {
    ...
    for (let n = 0; n < 8; n++) {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        if (i === n) expect(isHIGH(v), `dec(in=${n}) Y${i} → ${v}V (want HIGH)`).toBe(true);
        else expect(isLOW(v), `dec(in=${n}) Y${i} → ${v}V (want LOW)`).toBe(true);
      }
    }
  });

  it('4-bit magnitude comparator: A>B / A=B / A<B', { timeout: 30_000 }, async () => {
    ...
    async () => {
      ...
      for (const { A, B, M, result } of cases) {
        ...
        for (let i = 0; i < 4; i++) {
          ...
          if (want) expect(isHIGH(v), `${op}, bit ${i} → ${v}V HIGH`).toBe(true);
  ...
  it('1-bit ALU slice: mode select drives AND / OR / XOR / ADD', { timeout: 30_000 }, async () => {
    ...
    async () => {
      ...
      for (const { A, B, Ci } of cases) {
        ...
        for (let i = 0; i < 4; i++) {
          ...
          const want = (total >> i) & 1;
          if (want) expect(isHIGH(v), `CLA(${A}+${B}+${Ci}).S${i} → ${v}V HIGH`).toBe(true);
          else expect(isLOW(v), `CLA(${A}+${B}+${Ci}).S${i} → ${v}V LOW`).toBe(true);
        }
        const cout = (total >> 4) & 1;
