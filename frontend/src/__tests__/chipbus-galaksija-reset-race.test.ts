...
wire('rst', 'RESET', 'z80', 'RESET');
wire('rst', 'WAIT', 'z80', 'WAIT');
wire('rst', 'BUSREQ', 'z80', 'BUSREQ');
wire('rst', 'INT', 'z80', 'INT');
wire('rst', 'NMI', 'z80', 'NMI');

const STATE: ChipNetState = {
  wires: W,
  components: ['z80', 'rom', 'ram', 'inv', 'disp', 'rst'].map((id) => ({
    id,
...
describe.skipIf(!have)('chipbus Phase 3 — RESET ordering race via reset-gen', () => {
  ...
  it('RACE: reset-gen releases RESET before the Z80 exists — must still boot', async () => {
    ...
    const rst = await mk(pm, 'rst', 'rst', RST);
    rst.start();
    ...
    const z80 = await mk(pm, 'z80', 'z80', Z80);
    z80.start();
    ...
    expect(
      cellLit(fb!, 0, 1),
      'the BASIC ">" prompt rendered — the Z80 left reset',
    ).toBeGreaterThan(4);
