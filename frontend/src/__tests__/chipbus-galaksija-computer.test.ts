...
describe.skipIf(!have)('chipbus Phase 3 — full Galaksija computer renders READY', () => {
  ...
  it('boots the ROM and renders the READY prompt on the video display', async () => {
    ...
    const mk = async (
      k: keyof typeof paths,
      id: string,
      pins: string[],
      display?: { width: number; height: number },
    ) =>
      ChipInstance.create({
        wasm: new Uint8Array(readFileSync(paths[k])),
        componentId: id,
        pinManager: pm,
        wires: wiresFor(id, pins),
        display,
      });
    const z80 = await mk('z80', 'z80', Z80_PINS);
    z80.start();
    const rom = await mk('rom', 'rom', ROM_PINS);
    rom.start();
    const ram = await mk('ram', 'ram', RAM_PINS);
    ram.start();
    const inv = await mk('inv', 'inv', INV_PINS);
    inv.start();
    ...
    let litReady = 0;
    for (let y = 0; y < 8; y++)
      for (let x = 16; x < 56; x++) if (fb![(y * 256 + x) * 4 + 1] > 0x80) litReady++;
