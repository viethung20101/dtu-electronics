...
const sw = (id: string, v: 0 | 1 = 0): DigitalComponent => ({
  id,
  metadataId: 'slide-switch',
  properties: { value: v },
});
...
describe('digital-gate-engine Phase 5 — flip-flops', () => {
  ...
  it('JK flip-flop: hold / set / reset / toggle', () => {
    ...
    set(1, 0);
    pulse();
    expect(net.readNet(Q), 'J=1,K=0 set -> 1').toBe(1);
    set(0, 0);
    pulse();
    expect(net.readNet(Q), 'J=0,K=0 hold -> 1').toBe(1);
    set(0, 1);
    pulse();
    expect(net.readNet(Q), 'J=0,K=1 reset -> 0').toBe(0);
    set(1, 1);
    pulse();
    expect(net.readNet(Q), 'J=1,K=1 toggle -> 1').toBe(1);
    set(1, 1);
    pulse();
    expect(net.readNet(Q), 'J=1,K=1 toggle -> 0').toBe(0);
  });
