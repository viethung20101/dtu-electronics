...
const P = {
  ...
  ram: f('galaksija-ram.wasm'),
  inv: f('inverter.wasm'),
  disp: f('galaksija-display.wasm'),
  kbd: f('galaksija-keyboard.wasm'),
};
...
for (const i of range(13))
  for (const c of ['rom', 'ram', 'disp', 'kbd']) wire('z80', `A${i}`, c, `A${i}`);
wire('z80', 'A13', 'rom', 'CE');
wire('z80', 'A13', 'inv', 'IN');
wire('inv', 'OUT', 'ram', 'CE');
wire('z80', 'A13', 'disp', 'A13');
wire('z80', 'A13', 'kbd', 'A13');
for (const i of range(8))
  for (const c of ['rom', 'ram', 'disp', 'kbd']) wire('z80', `D${i}`, c, `D${i}`);
wire('z80', 'RD', 'rom', 'OE');
wire('z80', 'RD', 'ram', 'OE');
wire('z80', 'RD', 'kbd', 'RD');
wire('z80', 'WR', 'ram', 'WE');
...
const STATE: ChipNetState = {
  wires: W,
  components: ['z80', 'rom', 'ram', 'inv', 'disp', 'kbd'].map((id) => ({
    id,
    metadataId: 'custom-chip',
  })),
  boards: [],
};
const pk = (c: string, p: string): number =>
  resolveChipNetKey(STATE, c, p) ?? syntheticChipPin(c, p);
const wf = (c: string, pins: string[]) =>
  new Map(pins.map((p) => [p, pk(c, p)] as [string, number]));

// Lit pixels (bright green) inside character cell (col,row).
const cellLit = (fb: Uint8Array, col: number, row: number): number => {
  let n = 0;
  for (let y = 0; y < 8; y++)
    for (let x = 0; x < 8; x++) if (fb[((row * 8 + y) * 256 + (col * 8 + x)) * 4 + 1] > 0x80) n++;
  return n;
};
...
describe.skipIf(!have)('chipbus Phase 3 — typing on the Galaksija', () => {
  ...
  it('pressing A echoes "A" after the BASIC prompt', async () => {
    ...
    const mk = async (
      ...
      ChipInstance.create({
        wasm: new Uint8Array(readFileSync(P[k])),
        componentId: id,
        pinManager: pm,
        wires: wf(id, pins),
        display,
      });
    const z80 = await mk('z80', 'z80', Z80);
    z80.start();
    ...
    disp.onFramebufferUpdate((r) => {
      fb = r as Uint8Array;
    });
    disp.start();
    const kbd = await mk('kbd', 'kbd', KBD);
    kbd.start();

    for (const p of ['WAIT', 'INT', 'NMI', 'BUSREQ']) pm.triggerPinChange(pk('z80', p), true);
    pm.triggerPinChange(pk('z80', 'RESET'), false);
    pm.triggerPinChange(pk('z80', 'RESET'), true);
    ...
    const setKey = (off: number, down: number) =>
      (kbd.exports as { set_key: (o: number, d: number) => void }).set_key(off, down);
    setKey(1, 1);
